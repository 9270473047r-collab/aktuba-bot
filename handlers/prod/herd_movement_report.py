...
def aggregate_flow(reports: list[dict]) -> dict:
    ...
    breed_agg: Dict[Tuple[str, str], int] = {}  # (to, group) -> count
...
def build_text_report(daily: dict, month_flow: dict, year_flow: dict) -> str:
    ...
    # Поголовье
    total_cattle = int(daily.get("total_cattle", 0) or 0)
    ...
    # Переводы/поступления/племпродажа
    ...
    calves_total = (h_cows + b_cows + h_net + b_net)
    if calves_total != calv_total:
        warns.append(f"Несоответствие: приплод (тёлки+бычки={calves_total}) ≠ отёлы (коровы+нетели={calv_total}).")
    ...
    text = (
        ...
        f"• мертворождённые — <b>{fmt_int(stillborn)}</b>\n"
        ...
        f"• мертворождённые — <b>{fmt_int(still_m)}</b>\n"
        ...
        f"• мертворождённые — <b>{fmt_int(still_y)}</b>\n"
...
async def start_submit(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "✅ Начинаем сдачу отчёта <b>«Сводка по стаду»</b>.\n"
...
async def wizard_input(message: types.Message, state: FSMContext):
    ...
    # закончили основную часть — переходим к переводам
    if step_idx >= len(STEPS):
        ...
        await message.answer(
            "🔄 Были <b>переводы</b> из ЖК в другие подразделения?",
            parse_mode="HTML",
            reply_markup=kb_yes_no("tr_out")
        )
        return
...
async def tr_out_yesno(callback: types.CallbackQuery, state: FSMContext):
    ...
    if ans == "no":
        await callback.message.answer(
            "📥 Были <b>поступления</b> в ЖК из подразделений/источников?",
            parse_mode="HTML",
            reply_markup=kb_yes_no("tr_in")
        )
        await callback.answer()
        return
...
async def finalize_and_send(message: types.Message, state: FSMContext, user_id: int):
    ...
    # PDF 1: полная сводка за сутки (имя как требуете)
    date_ddmmyyyy = str(answers.get("report_date"))
    pdf1 = build_herd_daily_pdf_bytes(
        LOCATION_TITLE,
        date_ddmmyyyy,
        text,
    )
    fn1 = f"Сводка по стаду {LOCATION_TITLE} за {date_ddmmyyyy}.pdf"
    await message.answer_document(BufferedInputFile(pdf1, filename=fn1))

    # PDF 2: итоги месяца (движение + переводы/поступления/племпродажа агрегированно)
    month_label = f"{ddmmyyyy_from_iso(m_from)} — {ddmmyyyy_from_iso(m_to)}"
    pdf2 = build_herd_monthly_pdf_bytes(
        LOCATION_TITLE,
        month_label,
        month_flow,
    )
    fn2 = f"Сводка по стаду {LOCATION_TITLE} за месяц {month_label}.pdf"
    await message.answer_document(BufferedInputFile(pdf2, filename=fn2))
...
async def view_latest(callback: types.CallbackQuery):
    ...
    pdf1 = build_herd_daily_pdf_bytes(LOCATION_TITLE, date_ddmmyyyy, text)
    fn1 = f"Сводка по стаду {LOCATION_TITLE} за {date_ddmmyyyy}.pdf"
    await callback.message.answer_document(BufferedInputFile(pdf1, filename=fn1))

    # PDF 2: месяц
    month_label = f"{ddmmyyyy_from_iso(m_from)} — {ddmmyyyy_from_iso(m_to)}"
    pdf2 = build_herd_monthly_pdf_bytes(LOCATION_TITLE, month_label, month_flow)
    fn2 = f"Сводка по стаду {LOCATION_TITLE} за месяц {month_label}.pdf"
    await callback.message.answer_document(BufferedInputFile(pdf2, filename=fn2))
